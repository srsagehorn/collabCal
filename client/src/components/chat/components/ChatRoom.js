...
function ChatRoom() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setRoomname(room);
            firebaseConfig.database().ref('chats/').orderByChild('roomname').equalTo(roomname).on('value', resp => {
              setChats([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            setNickname(localStorage.getItem('nickname'));
            setRoomname(room);
            firebaseConfig.database().ref('roomusers/').orderByChild('roomname').equalTo(roomname).on('value', (resp2) => {
              setUsers([]);
    ...
    const exitChat = (e) => {
        ...
        firebaseConfig.database().ref('roomusers/').orderByChild('roomname').equalTo(roomname).once('value', (resp) => {
          let roomuser = [];
